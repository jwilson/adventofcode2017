main() {
  ...
  input.split('\n').forEach((line) {
    ...
    splitLine.forEach((word) {
      ...
      splitWord.sort();
      sortedWords.add(splitWord.join(''));
    });

    for (int i = 0; i < sortedWords.length; i++) {
      if (sortedWords.indexOf(sortedWords[i], sortedWords.indexOf(sortedWords[i]) + 1) != -1) {
        phrases++;
...
String input = """sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp

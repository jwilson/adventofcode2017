List<List<String>> balancingPrograms = new List<List<String>>();

main() {
  List<String> programs = input.split('\n');  
  programs.forEach((p) {
    if (p.indexOf('->') > 0) {
      List<String> programAndPrograms = p.split('->');
      List<String> programParts = new List<String>();
      programAndPrograms[0].split(' ').forEach((p) {
        if (p.contains('(')) {
          programParts.add(p.substring(p.indexOf('(') + 1, p.indexOf(')')));
        } else {
          programParts.add(p); 
        }        
      });
      programAndPrograms[1].split(', ').forEach((p) {
        programParts.add(p.trim());
      });
      balancingPrograms.add(programParts);
    }
  });

  String lowestProgram = '';
  
  for (var program in balancingPrograms) {
    bool foundMatch = false;
    for (var p2 in balancingPrograms) {
      if (p2[0] != program[0]) {
    ...
    if (!foundMatch) {
      lowestProgram = program[0];
    }
  }

  for (var p in balancingPrograms) {
    if (p[0] == lowestProgram) {
      findBadWeight(p); 
    }
  }  
}
...
findBadWeight(lowest) {
  int weight = int.parse(lowest[1]);
  for (int i = 3; i < lowest.length; i++) {
    int weight2 = getWeight(lowest[i]);
  }
}
...
String input3 = """uglvj (99) -> ymfjt, gkpgf
vvwrg (51)
qrpgt (5)
qhqbqj (55)
taxdaf (23)

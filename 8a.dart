main() {
  ...
  List<List<dynamic>> instructions = new List<List<dynamic>>();
  for (var instruction in input) {
    ...
    int incOrDec = int.parse(parts[2]);
    ...
    switch(parts[5]) {
      case '>':
        if (registers[parts[4]] > int.parse(parts[6])) {
          registers[parts[0]] += incOrDec;
        }
        break;
      case '<':
        if (registers[parts[4]] < int.parse(parts[6])) {
  ...
  List<int> registerValues = registers.values.toList();
  registerValues.sort();
  print(registerValues.reversed.first);
}
...
String input = """av inc 167 if f > -9
av inc 640 if uea == 0
fk dec -960 if tn > -9
